...
class Superuser extends CI_Controller {
	...
	public function __construct()
	{
		...
		if($this->tank_auth->get_user_level() != WMC_ROLE_ADMIN)
	...
	public function addLapangan()
	{
		...
		$this->form_validation->set_rules('pemilik', 'Pemilik',"trim|required|xss_clean|callback__array_match[pemilik]");
		...
		if($this->form_validation->run())
		{
			$dbdata = array(
				...
				'id_pemilik' => $this->form_validation->set_value('pemilik'), 
				'alamat' => $this->form_validation->set_value('alamat'), 
				'telp' => $this->form_validation->set_value('telp'), 

				);
			if(!is_null($this->lapangan->create_lapangan($dbdata)))
